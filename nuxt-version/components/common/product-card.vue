...
import type { ProductType } from "@/shared/type";
...
<template>
  ...
  >
    ...
    <div :class="cn('flex flex-col', vertical ? '' : 'grow')">
      ...
      <p class="text-[12px] text-gray-label line-clamp-1 mb-2">
        {{ item.description }}
      </p>
      <span
        :class="cn('text-primary text-sm', !vertical ? 'mb-[10px] flex' : '')"
      >
        ${{ item.price }}
      </span>

      <span
        class="flex items-center text-sm text-gray-label gap-[3px]"
        v-show="!vertical"
      >
        <Star />
        {{ item.rate }} Ratings
      ...
      <div
        class="flex justify-between items-center mt-[11px]"
        v-show="!vertical"
      >
        <div
        ...
        >
          <span class="text-[10px] text-gray-label">Size</span>
          <span
            v-for="size in item.sizes"
            :key="size"
            class="flex items-center justify-center px-2 py-[6px] border border-gray-border rounded-full text-[10px]"
          >
