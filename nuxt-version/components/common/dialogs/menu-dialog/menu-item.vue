...
import type { MenuTab } from "@/shared/type";
...
watch(open, () => {
  if (mainRef.value) {
    mainRef.value.style.height = `${
      open.value ? mainRef.value.scrollHeight : 0
    }px`;
  }
});
...
<template>
  <div :class="cn('overflow-hidden pt-5', open && 'pb-5')">
    <div
      class="flex gap-4 items-center justify-between pb-4 cursor-pointer"
      @click="onOpen"
    >
    ...
    <div
      class="flex flex-col gap-4 transition-all pl-5"
      ref="mainRef"
    >
      <NuxtLink
        :to="ci.link"
