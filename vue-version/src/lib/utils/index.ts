...
export const handleInputValue = (value: string, type: 'string' | 'number') => {
  ...
  return !value.length || (isNumeric && type === 'number') || (!isNumberInside && type === 'string')
}

export const formateCCV = (value: string) => value.replace(/[-/]/g, '').trim().slice(0, 4)
...
export const isMasterCard = (cardNumber: string) => /^5[1-5][0-9]{14}$/g.test(cardNumber)
