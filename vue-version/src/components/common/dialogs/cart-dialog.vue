...
<template>
  <Modal :isOpen="open">
    ...
    <div class="flex-grow flex items-center justify-center" v-show="!!!cartList?.length">
      <p class="text-4 text-[#888888] text-center">You have no items in your Shopping Bag.</p>
    </div>
    <div
      class="flex-grow flex flex-col gap-4 mt-5 h-full overflow-auto mb-[70px]"
      v-show="!!cartList?.length"
    >
      <CartItem :item="c" v-for="c in cartList" :key="c._id" />
      <div class="w-full h-[1px] bg-[#C4C4C4]"></div>
      <div class="flex justify-between items-center">
        <span class="text-sm tracking-[3px] uppercase">Sub total</span>
        <span class="text-4 tracking-[3px] uppercase text-[#A8715A]"> ${{ totalPrice }} </span>
      </div>
      <p class="text-[#888888] text-sm pr-3">
        *shipping charges, taxes and discount codes are calculated at the time of accounting.
      </p>
    </div>
    <Button class="absolute bottom-0 left-0 w-full" :onClick="onSubmit">
