...
<template>
  <div class="px-4 py-1 pt-[77px] flex items-center justify-between max-w-[1280px] mx-auto w-full">
    <span class="text-base uppercase leading-4 text-[#333333]">
    ...
    <div class="flex items-center gap-3">
      ...
      />
      <div
        class="flex items-center justify-center w-fit px-3 py-3 bg-[#c4c4c41a] rounded-[33px] cursor-pointer"
        @click="() => onFilterChange('view', filters.view == 'grid' ? 'list' : 'grid')"
      >
      ...
      <div
        class="flex items-center justify-center w-fit px-3 py-3 bg-[#c4c4c41a] rounded-[33px] cursor-pointer"
        @click="() => onFilterChange('dir', filters.dir == 'down' ? 'up' : 'down')"
      >
        <Sort :class="cn('size-5', filters.dir == 'down' ? 'fill-black' : 'fill-[#A8715A]')" />
      </div>
    </div>
  </div>
</template>
