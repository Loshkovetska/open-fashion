...
<template>
  <div class="my-8">
    <span class="uppercase text-base text-[#888888] mb-3 flex"> Shipping Method </span>
    <div class="relative">
      <div
        class="flex items-center justify-between py-[13px] px-5 bg-[#F9F9F9] rounded-[44px] cursor-pointer"
      >
        <div class="flex items-center justify-between flex-grow" @click="() => onOpen(!isOpen)">
          <span>{{ delivery.title }}</span>
          <span>{{ delivery.price == 'FREE' ? delivery.price : `$${delivery.price}` }}</span>
        </div>
        <ForwardIcon />
      </div>
      <div
