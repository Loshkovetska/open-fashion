...
const { withTotal = false, buttonText, disabled, onClick } = defineProps<BottomPartPropType>()
...
<template>
  <div class="fixed bottom-0 left-0 w-full">
    <div class="flex items-center justify-between px-4 py-6 bg-white" v-if="withTotal">
      <span class="text-sm tracking-[3px] uppercase">Total</span>
      <span class="text-4 tracking-[3px] uppercase text-[#A8715A]">
        $
        {{ totalPrice + (delivery.price === 'FREE' ? 0 : (delivery.price as number)) }}
      </span>
    </div>
    <ButtonUi :onClick="onClick" :class="disabled ? 'pointer-events-none opacity-50' : ''">
